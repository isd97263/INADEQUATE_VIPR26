function best_j12 = optimize_j12_mse()
    ...
    log_file = 'j12_simple_progress.txt';
    fid = fopen(log_file, 'w');
    fprintf(fid, 'J12 Optimization Started: %s\n', datestr(now));
    fprintf(fid, '========================================\n');
    fclose(fid);

    for restart = 1:num_restarts

        % randomly initialize J12 
        current_j12 = 10 + (30 - 10) * rand();
        velocity = 0;
        ...
        fprintf(fid, '\n--- RESTART %d/%d: Initial J12 = %.6f Hz ---\n', restart, num_restarts, current_j12);
        fclose(fid);

        for iter = 1:max_iterations

            calc_cost = @(j) compute_cost(j, J23_fixed, J13_fixed, exp_norm, f1_exp, f2_exp);

            current_cost = calc_cost(current_j12);

            if current_cost < restart_best_cost
                restart_best_cost = current_cost;
            end

            cost_plus = calc_cost(current_j12 + h_eff);
            cost_minus = calc_cost(current_j12 - h_eff);
            grad = (cost_plus - cost_minus) / (2 * h_eff);

            current_lr = learning_rate_init * (1 / (1 + 0.01 * iter));
            velocity = momentum * velocity - current_lr * grad;
            current_j12 = current_j12 + velocity;

            current_j12 = max(10, min(30, current_j12));

            fid = fopen(log_file, 'a');
            fprintf(fid, 'Iter %3d | J12: %6.6f Hz | Cost: %.6f \n', ...
                iter, current_j12, current_cost);
            fclose(fid);
        ...
        if restart_best_cost < best_overall_cost
            ...
            best_overall_j12 = current_j12;
        end
    end

    best_j12 = best_overall_j12;

    fid = fopen(log_file, 'a');
    fprintf(fid, '\n========================================\n');
    fprintf(fid, 'OPTIMIZATION COMPLETE.\n');
    fprintf(fid, 'Global Best J12: %.6f Hz\n', best_j12);
    fprintf(fid, 'Final Cost: %.6f\n', best_overall_cost);
    fprintf(fid, '========================================\n');
    fclose(fid);
    
    save('j12_simple_result.mat', 'best_j12', 'best_overall_cost');
end
...
function cost = compute_cost(j12, j23, j13, exp_norm, f1_exp, f2_exp)
    try
        ...
        mse = mean((sim_aligned(:) - exp_norm(:)).^2) * 100000000;
